...
export default function EmployeePage() {
  ...
  async function complete(eventId: string) {
    ...
    try {
      ...
      try { setRecommendations(await api.recommendations(employeeId)); }
  ...
  return <AppShell>
    ...
    {notice && <div role="status" className="mb-5 flex items-center gap-2 rounded-xl border border-[#c7ebd6] bg-[#f0f3ff] px-4 py-3 text-sm font-semibold text-[#087d48]"><Check size={16}/>{notice}</div>}
    {loading ? <div className="space-y-5" role="status"><p className="muted text-sm">Знакомимся с вашим профилем…</p><div className="skeleton h-56"/><div className="grid-main"><div className="skeleton h-96"/><div className="skeleton h-96"/></div></div> : !profile ? <div className="card p-10 text-center"><CircleHelp className="mx-auto text-[#006b32]" size={32}/><h2 className="mt-4 text-lg font-bold">Пока не удалось открыть профиль</h2><p className="muted mt-2 text-sm">Попробуйте ещё раз или выберите другой профиль через «Сменить профиль».</p></div> : <>
      <div className="card profile-hero relative overflow-hidden p-6 md:p-8"><div className="absolute right-0 top-0 h-52 w-52 rounded-full bg-[#e5f8ed] blur-3xl"/><div className="relative grid gap-8 md:grid-cols-[1fr_250px] md:items-center"><div><div className="pill"><UserRound size={13}/> ВАША ОТПРАВНАЯ ТОЧКА</div><h2 className="mt-5 text-[27px] font-semibold tracking-tight">Здравствуйте, {profile.name}!</h2><p className="mt-1 text-sm text-[#65756a]">{profile.role}</p><div className="mt-6 flex flex-wrap gap-3"><span className="rounded-lg bg-[#f0f3ff] px-3 py-2 text-xs font-bold text-[#485b4e]">{profile.currentGrade} <ArrowRight className="inline mx-1 text-[#006b32]" size={13}/> {profile.targetGrade}</span><span className="rounded-lg bg-[#f0f3ff] px-3 py-2 text-xs font-bold text-[#485b4e]"><Clock3 className="mr-1 inline" size={13}/> {profile.tenureMonths} мес. в компании</span></div><div className="mt-7 flex items-center gap-3 text-sm text-[#427957]"><TrendingUp size={17}/><span><b>{completed}</b> из {profile.skills.length} навыков на целевом уровне</span></div></div><div className="flex flex-col items-center rounded-2xl border border-[#e2e8f8] bg-white/80 p-5"><div className="relative grid h-36 w-36 place-items-center rounded-full" style={{ background: `conic-gradient(#006b32 ${ready}%, #e2e8f8 0)` }}><div className="grid h-[114px] w-[114px] place-items-center rounded-full bg-white"><div className="text-center"><motion.div key={ready} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="text-[35px] font-semibold tracking-tight">{ready}%</motion.div><div className="-mt-1 text-[10px] font-bold uppercase tracking-wider text-[#7e8c80]">готовность</div></div></div></div><div className="mt-3 text-center text-xs text-[#738275]">к грейду {profile.targetGrade}</div></div></div></div>
      <div className="grid-main development-grid mt-6"><section className="card p-6"><div className="flex items-center justify-between"><div><div className="eyebrow">КАРТА НАВЫКОВ</div><h2 className="section-title mt-1">Что поможет двигаться дальше</h2></div><div className="hidden rounded-xl bg-[#f0f3ff] p-3 text-[#006b32] sm:block"><Target size={21}/></div></div><div className="mt-6 space-y-7">{hard.length > 0 && <div><div className="mb-3 flex items-center justify-between text-xs font-semibold uppercase tracking-wider text-[#67786d]"><span>Профессиональные навыки</span><span>{hard.length}</span></div><div className="grid gap-3 sm:grid-cols-2">{hard.map((skill) => <SkillCard key={skill.skillId} skill={skill}/>)}</div></div>}{soft.length > 0 && <div><div className="mb-3 flex items-center justify-between text-xs font-semibold uppercase tracking-wider text-[#67786d]"><span>Гибкие навыки</span><span>{soft.length}</span></div><div className="grid gap-3 sm:grid-cols-2">{soft.map((skill) => <SkillCard key={skill.skillId} skill={skill}/>)}</div></div>}{profile.skills.length === 0 && <p className="muted text-sm">Навыки пока не загружены.</p>}</div></section><section><div className="mb-4 flex items-center justify-between"><div><div className="eyebrow">С УЧЁТОМ ВАШЕГО ОПЫТА</div><h2 className="section-title mt-1">С чего начать</h2></div><Sparkles size={20} className="text-[#006b32]"/></div><p className="muted mb-4 text-sm leading-6">AI учитывает навыки, историю участия и цель. Выберите то, что подходит вам сейчас.</p><div className="space-y-4">{recommendationsLoading ? <div role="status" className="card p-6"><LoaderCircle size={20} className="animate-spin text-[#006b32]"/><h3 className="mt-3 font-bold">Подбираем подходящие шаги</h3><p className="muted mt-2 text-sm">Это может занять несколько секунд. Пока можно посмотреть свои навыки.</p></div> : recommendationsError ? <div role="alert" className="card p-6"><p className="text-sm leading-6">{recommendationsError}</p><button className="btn-secondary mt-4" disabled={busyEvent !== null} onClick={() => setReload((value) => value + 1)}>Попробовать ещё раз</button></div> : recommendations.length > 0 ? recommendations.map((recommendation) => <RecommendationCard key={recommendation.eventId} recommendation={recommendation} busy={busyEvent !== null} saving={busyEvent === recommendation.eventId} onComplete={complete}/>) : <div className="card p-7 text-center"><Award className="mx-auto text-[#006b32]" size={30}/><h3 className="mt-3 font-bold">Пока нет подходящих шагов</h3><p className="muted mt-2 text-sm">Среди доступных активностей пока нечего предложить. Можно обсудить следующий шаг с руководителем или HR.</p></div>}</div></section></div>
      <div className="mt-6 flex items-start gap-3 rounded-2xl border border-[#e2e8f8] bg-[#f0f3ff] p-5"><div className="rounded-lg bg-white p-2 text-[#006b32]"><Lightbulb size={18}/></div><div><div className="text-sm font-bold text-[#246943]">Прогресс — ориентир, а не оценка вас</div><p className="mt-1 text-xs leading-5 text-[#5b7c66]">Готовность показывает, какая часть требований к навыкам уже выполнена. Уровень выше цели не добавляет лишних процентов. Решение о повышении обсуждается отдельно с руководителем.</p></div></div>
    </>}
  </AppShell>;
}
